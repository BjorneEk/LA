...
void LA_test_mult_advanced(int max) {
  LA_matrix *m1, *m2, *res;
  FILE *fp_1, *fp_2;
  long * times, * times_naive;
  clock_t begin, end;
  int i, dim;

  times = (long*)malloc(max * sizeof(long));
  times_naive = (long*)malloc(max * sizeof(long));
  assert(times != NULL && times_naive != NULL, "Out of memory");

  for (i = 1; i <= max; i++) {
    if(random_d(0, 1) > 0.5) dim = i;
    else dim = (int) (random_d(1, 400));
    ...
    times[i-1] = (long)(1000000.0*(double)(end - begin) / CLOCKS_PER_SEC);
    LA_free(res);
    ...
    times_naive[i-1] = (long)(1000000.0*(double)(end - begin) / CLOCKS_PER_SEC);
